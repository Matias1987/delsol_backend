...
const obtenerDescuentosCliente = ({ idcliente }, callback) => {
  const query = `SELECT 
dc.porcentaje, 
dc.id_descuento, 
dc.fk_cliente, 
...
dc.activo 
FROM 
descuento_cliente dc, 
subgrupo sg 
WHERE 
sg.idsubgrupo = dc.fk_subgrupo AND  
dc.fk_cliente=${idcliente};`;
  doQuery(query, (response) => {
    return callback(response.data);
  });
};
...
const agregarDescuentoClienteSubgrupo = (data, callback) => {
  ...
  const query = descuentoGral
    ? `INSERT INTO descuento_cliente (fk_cliente, fk_subgrupo, porcentaje, todos) VALUES (NULL, ${idsubgrupo}, ${porcentaje}, 1)`
