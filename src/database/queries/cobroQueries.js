...
const queryAgregarCobroV2 = (idcaja, data) => `insert into cobro (            
                ...
                ${parseFloat(data.monto) - parseFloat(data.mp?.ctacte_monto ?? "0") /* subtract ctacte monto */}, 
                ${escapeHelper(data.tipo)},
                ${escapeHelper(data.sucursal_idsucursal)},
                date('${data.fecha}')
                )`;

const queryUpdateVentaMontos = (data, total) =>
  `UPDATE venta  v SET v.descuento=${data.descuento}, v.debe=v.subtotal-${data.descuento},  v.monto_total=v.subtotal-${data.descuento},  v.haber=v.haber + ${total}, v.saldo = v.saldo - ${total} WHERE v.idventa=${data.idventa || "0"};`;
...
const queryRemoveMPRows = (data) =>
  `DELETE FROM venta_has_modo_pago vhmp WHERE vhmp.venta_idventa=${data.idventa};`;
const queryRemoveCtaCteRows = (data) =>
  `DELETE FROM venta_has_modo_pago vhmp WHERE vhmp.modo_pago = 'ctacte' and vhmp.venta_idventa=${data.idventa};`;
