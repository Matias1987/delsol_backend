...
const agregar_comprobante= (data, callback) => {
  const connection = mysql_connection.getConnection();
  connection.connect();
  connection.query(
    `INSERT INTO comprobante (numero, proveedor_idproveedor, monto, cantidad) VALUES ('${data.numero}', ${data.proveedor_idproveedor},${data.monto},${data.cantidad});`,
    (err, result) => {
...
const detalle_comprobante= (data, callback) => {
  const connection = mysql_connection.getConnection();
  connection.connect();
...
const lista_elementos_comprobante_= (data, callback) => {
  const connection = mysql_connection.getConnection();
  connection.connect();
  connection.query(
    `SELECT 
...
const agregar_comprobante_v2 = (data, callback) => {
  ...
  let query_factura = `INSERT INTO comprobante (numero, proveedor_idproveedor, monto, cantidad, tipo, punto_venta, es_remito, fecha, descuento, c_n_gravados, imp_int) VALUES (
...
const agregar_comprobante_v3 = (data, callback) => {
  console.log(JSON.stringify(data));
  if (data.nro === "" || +data.fkproveedor === -1) {
    console.log("Error en datos de factura");
    callback(-1);
  ...
  let query_factura = `INSERT INTO comprobante (numero, proveedor_idproveedor, monto, cantidad, tipo, punto_venta, es_remito, fecha, descuento, c_n_gravados, imp_int, neto_gravado, fk_moneda) VALUES (
    ${connection.escape(data.nro)}, 
    ${connection.escape(data.fkproveedor)},
    ${connection.escape(data.total)},
    ${connection.escape(data.cant_productos)},
...
const obtener_comprobante_por_nro = (nro, callback) => {
  const query = `select * from comprobante f where f.numero = ${nro}}`;
  const connection = mysql_connection.getConnection();
  connection.connect();
...
const obtener_comprobante_filtros = (data, callback) => {
  ...
  const query = `SELECT DATE_FORMAT(f.fecha,'%d-%m-%y') AS 'fecha_f', f.*, p.nombre AS 'proveedor' FROM comprobante f, proveedor p WHERE
...
const obtener_comprobante_montos_adic = (idfactura, callback) => {
  const query = `SELECT 
                fi.monto, 
                'IVA' AS 't', 
...
const obtener_comprobante_saldo = (data, callback) => {
...
module.exports = {
  obtener_comprobante_filtros,
  obtener_comprobante_por_nro,
  obtener_comprobante,
  agregar_comprobante,
  detalle_comprobante,
  lista_elementos_comprobante_,
  agregar_comprobante_v3,
  obtener_comprobante_montos_adic,
  obtener_comprobante_saldo,
};
