...
const obtenerPagaresCliente = (data,callback) => {
    const query = `SELECT c.idcliente, c.apellido, c.nombre , p.* FROM pagare p, cliente c, venta v  WHERE p.fk_operacion = v.idventa AND c.idcliente = v.cliente_idcliente AND c.idcliente=${data.idcliente};`
    const connection = mysql_connection.getConnection();
    connection.connect()
    connection.query(query,(err,resp)=>{
        callback(resp)
