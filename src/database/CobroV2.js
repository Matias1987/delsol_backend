...
const { doQuery, escapeHelper, doTransaction } = require("./helpers/queriesHelper");
...
const agregar_cobro = (data, callback) => {
    ...
    const q_removeMPRows = data.removeMPRows && +data?.removeMPRows == 1 ? cobro_queries.queryRemoveMPRows(data) : "";
    const q_removeCtaCteRow = data.removeCtaCteRow && +data?.removeCtaCteRow == 1 ? cobro_queries.queryRemoveCtaCteRows(data) : "";
    ...
    doQuery2(
        obtenerCajaAbierta(data.sucursal_idsucursal), ((err, rows) => {
            ...
            doQuery2(query_agregar_cobro, (err1, result1) => {
                ...
                const update_venta_montos_query = cobro_queries.queryUpdateVentaMontos(data, total);
...
const insert_cobro_transaction = (data, callback) => {
    ...
    const __logic = async (connection) => {
        ...
        if (data.monto == data.mp.ctacte_monto && data.mp.ctacte_monto > 0) {
            ...
            await connection.query(`UPDATE venta  v SET v.descuento=${data.descuento}, debe=v.subtotal-${data.descuento},  monto_total=v.subtotal-${data.descuento}  WHERE v.idventa=${data.idventa};`);
            return;
        }
        ...
        const cajaResponse = await connection.query(obtenerCajaAbierta(data.sucursal_idsucursal));
        const idcaja = cajaResponse[0].idcaja;
        const cobroResponse = await connection.query(cobro_queries.queryAgregarCobroV2(idcaja, data));
    ...
    doTransaction(__logic,({response, error})=>{
        if(error){
            console.log("error")
