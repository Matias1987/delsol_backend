...
const agregar_stock = (data, callback) => {
  ...
  const values = [
    [data.sucursal_idsucursal, data.codigo_idcodigo, data.cantidad],
  ];
...
const agregar_stock_lote = (data, callback) => {
  ...
  data.codigos.forEach((r) => {
    _values +=
      (_values.length > 0 ? "," : "") +
      `(${data.subgrupo},'${r.codigo}','descripcion',${r.costo})`; // "("+data.subgrupo,r.codigo,"descripcion",r.costo+")";
  ...
  const query_insert_codigos =
    `INSERT ignore INTO codigo  (subgrupo_idsubgrupo, codigo, descripcion, costo) VALUES` +
    _values;
...
const descontar_cantidad_por_codigo = (_data, callback) => {
  ...
  connection.query(
    `SELECT c.idcodigo FROM stock s, codigo c WHERE c.idcodigo = s.codigo_idcodigo AND 
...
const obtener_lista_stock_filtros = (data, callback) => {
  ...
  if (typeof data.order !== "undefined") {
    switch (data.order) {
      case "alf_asc":
        order = " order by c.codigo asc";
        break;
      case "alf_desc":
        order = " order by c.codigo desc";
        break;
      case "precio_desc":
        order = " order by c.precio desc";
        break;
      case "precio_asc":
        order = " order by c.precio asc";
        break;
      case "cantidad_asc":
        order = " order by c.cantidad asc";
        break;
      case "cantidad_desc":
        order = " order by c.cantidad desc";
        break;
    }
  }
...
const obtener_stock_ventas = (filters, callback) => {
  ...
  const idcodigo =
    typeof filters.idcodigo == "undefined" ? "-1" : filters.idcodigo;
  ...
  const __filtro =
    typeof filters.filtroCod === "string" ? filters.filtroCod : "";
  ...
  __parts.forEach((r) => {
    __filtros_codigo +=
      (__filtros_codigo.length < 1 ? "" : " and ") + `c.codigo like '%${r}%' `;
  });
  __parts.forEach((r) => {
    __filtros_desc +=
