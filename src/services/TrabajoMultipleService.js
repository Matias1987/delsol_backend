...
const procesarTrabajoMultiple = (_data, callback) => {
  console.log("data received: " + JSON.stringify(_data));
  cajaService.obtener_caja(_data.fksucursal, (response) => {
    
    console.log("Getting caja response: " + JSON.stringify(response));
    ...
    controlarStock(products_quantities, data.idsucursal, (response) => {
      ...
      db.transaccionAgregarTM(data,(response)=>{
        callback({ ok: 1, data: response });
      })
      /*
          ...
          (response1) => {
            ...
            descontarStock(idVenta, data.idsucursal, (response2) => {
              if (response2.error) {
                return callback({
                  error: 1,
                  msg: "error when trying to discount stock quantities",
                });
              }
              ...
              obtenerTrabajoMultiple({ idventa: idVenta }, (response3) => {
                if (response3.error) {
                  return callback({
                    error: 1,
                    msg: "error fetching venta with trabajos",
                  });
                }
    ...
    trabajos.forEach((t) => {
      ...
      if (cur_idtrabajo) {
        cur_trabajo.items.push({
          ...
          detalle_trabajo: t.detalle_trabajo || "",
        });
      }
    });
...
const marcar_como_entregado = (data, callback) => {
  db.marcar_entregado(data, (response) => {
