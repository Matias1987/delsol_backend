...
const agregarCodigo = (req, res) => {
  ...
  codigoService.agregarCodigo(nuevo_codigo, (id) => {
    if (id < 0) {
...
const obtener_codigos_categoria = (req, res) => {
  const {
    params: { idfamilia, idsubfamilia, idgrupo, idsubgrupo, modo_precio },
  } = req;
...
const ejemplo_codigo = (req, res) => {
  const {
    params: { idsubgrupo, idgrupo },
  } = req;

  codigoService.ejemplo_codigo(
    { idsubgrupo: idsubgrupo || "0", idgrupo: idgrupo || "0" },
    (response) => {
      res.status(201).send({ status: "OK", data: response });
    },
  );
...
const obtenerStockItems = (req, res) => {
  codigoService.obtenerStockItems(null, (response) => {
    res.status(201).send({ status: "OK", data: response });
  });
};
const obtenerBasesItems = (req, res) => {
  codigoService.obtenerBasesItems(null, (response) => {
    res.status(201).send({ status: "OK", data: response });
  });
};
